    ...
    private IEnumerable<Product> Get(string file)
    {
        ...
        while (!string.IsNullOrWhiteSpace(worksheet.Cell(i, 1).Value.ToString()))
        {
            products.Add(new Product(
                Convert.ToInt16(worksheet.Cell(i, 1).Value.ToString()), 
                worksheet.Cell(i, 2).Value.ToString(), 
                worksheet.Cell(i, 3).Value.ToString(), 
                Convert.ToDecimal(worksheet.Cell(i, 4).Value.ToString())));
            i++;
        }
        return products;
    }
